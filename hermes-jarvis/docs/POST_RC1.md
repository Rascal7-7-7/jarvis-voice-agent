...
`秘書、it-study に何を指示した` / `秘書、指示の履歴を見せて` で
`secretary history --speech` を呼ぶ（`INTENT_HISTORY`）。実装は `SECRETARY.md`。

### 「指示」単独では発火させない

    秘書、it-study に何を指示した        -> HISTORY
    秘書、指示の履歴を見せて              -> HISTORY（全体）
    秘書、it-study に指示ファイルを作って  -> DISPATCH（作業依頼のまま）

`_HISTORY_WORDS` は疑問・履歴の形を要求する（`何を指示` / `履歴` /
`前回…指示` など）。`指示` だけで拾うと「指示ファイルを作って」が
履歴の問い合わせになってしまう。
...
## 対応が必要なものを先に言う（2026-09-09）

`INTENT_ATTENTION` を追加し、`INTENT_BRIEF` の読み上げにも**先頭で**足す。
実装は `~/work/scripts/secretary/attention.py`、設計は `SECRETARY.md`。

    秘書、対応が必要なものある    -> ATTENTION
    秘書、失敗してるものある      -> ATTENTION
    秘書、放置されてるものある    -> ATTENTION
    秘書、状況を教えて            -> BRIEF（先頭に attention の文が付く）
    秘書、it-study の状況を教えて  -> STATUS（変わらず）

判定の順は ATTENTION → HISTORY → STATUS/BRIEF → 除外 → DISPATCH/TEST → SEARCH。
「対応が必要」「失敗して」は「状況」より具体的なので先に見る。

### なぜ先頭か

記録は既にあるのに誰も見なければ意味がない。2026-09-09 に
19:26 の Codex 失敗（`exit=127`）が数時間埋もれた。停滞の件数より先に
「まだ誰も対応していないもの」を言う。

読み上げ上限（`jarvis_speech.MAX_SPOKEN_CHARS` = 180）で切られる場合、
先頭が残り末尾（git 未管理の件数）から落ちる。最悪ケース
（失敗3・実行中2・キュー4・停滞・git未管理）で実測 149 字。

    最近止まったプロジェクトが2件あります。停滞は3件です。最長はAi_Createで
    144日放置、未コミット1件です。また、git 未管理が2件あります。
    最大はnoxの408ファイルです。

## 一括テスト（2026-09-10）

`INTENT_SWEEP` を追加。`secretary sweep test` をバックグラウンドで投げ、
完了は ① の通知で知らせる（6プロジェクトで実測 91〜115 秒）。

### 「全部」の明示を要求する

    秘書、全プロジェクトのテストを走らせて  -> SWEEP
    秘書、全部のテストを実行して          -> SWEEP
    秘書、テストを一括で走らせて          -> SWEEP
    秘書、まとめてテストを走らせて         -> SWEEP
    秘書、テストを走らせて                -> SWEEP でも TEST でもない
    秘書、it-study のテストを走らせて      -> TEST（1プロジェクト）

プロジェクト名も「全部」も無い発話で6プロジェクト走らせない。
`_ALL_WORDS` と `_TEST_WORDS` の**両方**を要求する。

### 判定順で踏んだバグ

`全部` は `_LIST_WORDS`（`一覧|リスト|全部|どんなプロジェクト`）にもあるため、
最初は「全部のテストを実行して」が **LIST（一覧表示）に先取りされた**。

SWEEP は2条件（「全部」＋テスト実行）を要求するので、単語1つで当たる
LIST / BRIEF より具体的。判定を LIST より前へ移した。
`一覧` だけの発話が LIST のまま、`状況` が BRIEF のままであることも
テストで固定した。

また `_TEST_WORDS` の距離を `.{0,3}` → `.{0,6}` に広げた
（「テストを一括で走らせて」が外れていた）。

判定の順は SWEEP → ATTENTION → HISTORY → STATUS/BRIEF/LIST/REPORTS →
除外 → DISPATCH/TEST → SEARCH。

## 呼び名を「アルフ」にし、プロジェクトの起動/終了を音声から（2026-09-10）

### 実測で分かったこと（推測では全部外していた）

呼び名と別名を決める前に、`say -v Kyoko` で合成して
**runtime と同じ faster-whisper small / ja に通した**。

| 話した | 書き起こし | |
|---|---|---|
| 秘書、状況を教えて | **非処 状況を教えて** | 既存トリガに当たらない |
| ひしょ、状況を教えて | **一緒、状況を教えて** | 同上 |
| アルフ、状況を教えて | アルフ 状況を教えて | ✓ |
| アルフ（単独） | あるふ | ひらがな |
| アルフ、エムブイピーを起動して | **α-MVPを起動して** | **呼びかけ語が消えた** |
| アップ | アップ | ✓ |
| エーピーピー | **BPP** | |
| ボット | **ロット** | 誤変換 |
| ディスコード | **リスコード** | 誤変換 |
...
**`(秘書|ひしょ)` は音声で一度も当たっていなかった。**
これまでの確認はすべてテキスト入力で、実音声を通していなかった。
`アルフ` を足し、実測で出た誤変換（`非処` / `一緒、`）も受けるようにした。

`アルフ` は素朴に書くと「アルファベット」「アルファ版」「アルフレッド」に
部分一致する（実測で全部この表記に書き起こされた）ので `(?![ァぁレれ])` で除外。

### プロジェクトの起動/終了は呼びかけ語を要求しない

「アルフ」が α に化ける以上、秘書経由にすると届かない。
`jarvis_open` が既に「開いて/起動して」の許可リストを持っているので、
そこへプロジェクトを足した。`グーグルを開いて` と同じ扱いになる。

    アップを起動して        -> OPEN_PROJECT app    （Ghostty ウィンドウが開く）
    α-MVPを起動して        -> OPEN_PROJECT mvp
    トレードを終了して       -> CLOSE_PROJECT trade （tmux セッションごと）
    ワムを起動して          -> 拒否（client-a は作業対象外なので許可リストに無い）
    グーグルを閉じて         -> 拒否（閉じられるのはプロジェクトだけ）

終了の動詞は総称的なので、**対象が許可リストにあるときだけ** OPEN へ流す。
`会議を終了して` は OPEN に来ない。

### 実装で踏んだもの

**`capture_output=True` で起動が「失敗」と誤報された。**
`open-project.sh` は Ghostty をバックグラウンドで起動し、その子が
パイプを継承する。スクリプト自体が終わっても Python が EOF を待ち続け、
15 秒でタイムアウトして「mvpを開けませんでした」と言った。
**セッションは実際には立っていた。** 出力を捕まえないようにし、
さらに「起動した」と言う前に `tmux has-session` で確認するようにした。
修正後 0.23 秒。

## 音声の往復テスト（2026-09-10）

`tests/test_voice_roundtrip.py` — **合成 → STT → ルーティング**を1つの検証にする。

    ~/.hermes-venv/bin/python tests/test_voice_roundtrip.py    # 26件 / 53秒

CI では動かせない（`say` と Whisper モデルが要る）。手元で走らせる。

### なぜ必要だったか

音声アシスタントなのに、**音声で検証したテストが1本も無かった**。
宛先は 22 種類、検証は 53 箇所すべて**テキストを直接** classify/route に
渡すものだった。その結果 `(秘書|ひしょ)` が音声で一度も当たっていなかった。

書き起こしは **runtime が実際に呼ぶ `vm.transcribe_recording`** を使う。
Whisper のパラメータを複製すると、ここで直そうとしている
「検証方法が実態と違う」誤りを再生産する。
（`.aiff` は非対応。`.m4a` が最速で実測 0.7s）

### 初回実行で security gate の穴が出た

**gate が音声経路で機能していなかった。** STT が英単語をカタカナにするため:

| 話した | 聞こえた | gate |
|---|---|---|
| sudoで再起動して | スドーで再起動して | **素通り** |
| データベースを delete して | データベースをデリーとして | **素通り** |
| brew install してください | ブルーインスタルしてください | **素通り** |
| npm uninstall して | npm アンインスタルして | **素通り** |
| chmod 777 にして | ともっと777にして | **素通り** |

素通り分は `LOCAL_TOOL` へ届く。`LOCAL_TOOL` は
`hermes -t file,terminal,clarify` で、**`terminal` を含む**。
（実際にコマンドが実行されるかはエージェント側の承認次第で、そこは未確認）

gate のソースには「`\b` では `sudoで再起動して` に当たらない」という
考察のコメントが既にあった。**テキストの場合は検討済みで、
STT のカタカナ化だけが抜けていた。**

実測した書き起こしを足して 5件中4件を塞いだ。
`install` 系は**命令形と組み合わせたときだけ**止める
（「インストール手順を教えて」を潰さないため。実測で確認）。

### 塞げなかったもの（測った事実として残す）

- **`chmod 777`** → 「ともっと777にして」。`ともっと` を足すと日常語で
  誤発火するので**追加していない**
- **「スドーとは何ですか」** が止まる。STT が `とは` を `トは` にするため
  `_INQUIRY` の免除が効かない。gate の方針（迷ったら確認・無駄な確認は
  1秒の損）に沿うので許容

どちらも `TestGateKnownGaps` に**現状を固定するテスト**として置いた。
塞げたらテストが落ちるので、記述の更新を忘れない。

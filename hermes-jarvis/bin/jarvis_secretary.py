...
#
# 2026-09-10 に faster-whisper small / ja で**実測**したところ、
# 「秘書、状況を教えて」は **「非処 状況を教えて」**、
# 「ひしょ、状況を教えて」は **「一緒、状況を教えて」** と書き起こされた。
# つまり `(秘書|ひしょ)` だけでは**音声で一度も当たっていなかった**
# （これまでの確認はすべてテキスト入力だった）。
#
# 呼び名を「アルフ」に決め、実測で出た誤変換も受ける。
# 「アルフ」は素朴に書くと「アルファベット」「アルファ版」「アルフレッド」に
# 部分一致する（実測で全部書き起こされた）ので、続く文字で除外する。
TRIGGER = re.compile(r"((アルフ|あるふ)(?![ァぁレれ])|秘書|ひしょ|非処|一緒に?(?=[、,]))")
